...
export default function WahaConnectionWizard({ onStatusChange }: { onStatusChange?: (status: string) => void }) {
  ...
  const checkStatus = async () => {
    try {
      const data: SessionInfo = await apiFetch<SessionInfo>("/waha/status");
      setSessionInfo(data);
      setErrorMsg(null);
  ...
  const handleStartSession = async () => {
    ...
    setErrorMsg(null);
    try {
      await apiFetch("/waha/start", { method: "POST" });
      await checkStatus();
    } catch (e: any) {
      setErrorMsg(`Erro ao iniciar sessão: ${e.message || "Falha de conexão com a API"}`);
    } finally {
  ...
  const confirmStopSession = async () => {
    ...
    setErrorMsg(null);
    try {
      await apiFetch("/waha/stop", { method: "POST" });
      await checkStatus();
    } catch (e: any) {
      setErrorMsg(`Erro ao encerrar sessão: ${e.message || "Falha ao desconectar"}`);
    }
  };
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              {isConnected && (
                <Button variant="destructive" size="sm" onClick={() => setShowConfirmDisconnect(true)}>
              ...
              {needsQR ? (
                <div className="flex flex-col md:flex-row items-center gap-6">
                  <div className="rounded-xl border bg-muted/50 p-4">
                    ...
                    <img
                      src={`${API_BASE_URL}/waha/qr?t=${qrKey}`}
      ...
      <Dialog open={showConfirmDisconnect} onOpenChange={setShowConfirmDisconnect}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              A sessão atual do WhatsApp será encerrada. O sistema deixará de capturar mensagens automaticamente até que uma nova conexão seja estabelecida.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button variant="destructive" onClick={confirmStopSession}>
